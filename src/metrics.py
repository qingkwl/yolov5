...
class ConfusionMatrix:
    ...
    def __init__(self, nc, conf=0.25, iou_thres=0.45):
        self.matrix = np.zeros((nc + 1, nc + 1), dtype=np.int32)
    ...
    def process_batch(self, detections, labels):
        ...
        if detections is None:
            gt_classes = labels.astype(np.int32)
...
def _nms(xyxys, scores, threshold, time_limit=-1, sample_idx=0):
    ...
    while order.size > 0:
        ...
        if 0 < time_limit < (time.time() - s_time):
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False,
                        multi_label=False, labels=(), time_limit=20.0):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        i = nms_func(boxes, scores, iou_thres, _time_limit, xi)  # NMS for per sample
...
def scale_coords(img1_shape, coords, img0_shape, ratio_pad=None) -> np.ndarray:
