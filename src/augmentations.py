...
def random_perspective(img, targets=(), segments=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
    ...
    # Center
    _center = np.eye(3)
    _center[0, 2] = -img.shape[1] / 2  # x translation (pixels)
    _center[1, 2] = -img.shape[0] / 2  # y translation (pixels)

    # Perspective
    _perspective = np.eye(3)
    _perspective[2, 0] = random.uniform(-perspective, perspective)  # x perspective (about y)
    _perspective[2, 1] = random.uniform(-perspective, perspective)  # y perspective (about x)

    # Rotation and Scale
    _rotation = np.eye(3)
    a = random.uniform(-degrees, degrees)
    s = random.uniform(1 - scale, 1 + scale)
    _rotation[:2] = cv2.getRotationMatrix2D(angle=a, center=(0, 0), scale=s)
    ...
    # Translation
    _translation = np.eye(3)
    _translation[0, 2] = random.uniform(0.5 - translate, 0.5 + translate) * width  # x translation (pixels)
    _translation[1, 2] = random.uniform(0.5 - translate, 0.5 + translate) * height  # y translation (pixels)

    # Combined rotation matrix
    _matrix = _translation @ _shear @ _rotation @ _perspective @ _center  # order of operations (right to left) is IMPORTANT
    if (border[0] != 0) or (border[1] != 0) or (_matrix != np.eye(3)).any():  # image changed
        if perspective:
            img = cv2.warpPerspective(img, _matrix, dsize=(width, height), borderValue=(114, 114, 114))
        else:  # affine
            img = cv2.warpAffine(img, _matrix[:2], dsize=(width, height), borderValue=(114, 114, 114))
